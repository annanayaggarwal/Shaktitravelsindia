...
const AdminDashboard = () => {
  ...
  const fetchPackages = async () => {
    setIsLoading(true);
    try {
      const response = await axios.get('https://tourandtravelsbacked-production.up.railway.app/api/packages', {
  ...
  const fetchQueries = async () => {
    try {
      const response = await axios.get('https://tourandtravelsbacked-production.up.railway.app/api/queries', {
        headers: { 'x-auth-token': localStorage.getItem('token') }
      });
  ...
  const handleAddPackage = async (newPackage) => {
    try {
      const response = await axios.post('https://tourandtravelsbacked-production.up.railway.app/api/packages/header', newPackage, {
        headers: { 'x-auth-token': localStorage.getItem('token') }
      });
  ...
  const handleUpdatePackage = async (updatedPackage) => {
    try {
      const response = await axios.put(`https://tourandtravelsbacked-production.up.railway.app/api/packages/${updatedPackage._id}`, updatedPackage, {
        headers: { 'x-auth-token': localStorage.getItem('token') }
      });
  ...
  const handleDeletePackage = async (id) => {
    try {
      await axios.delete(`https://tourandtravelsbacked-production.up.railway.app/api/packages/${id}`, {
        headers: { 'x-auth-token': localStorage.getItem('token') }
      });
          ...
          <ul>
            {queries.map((query) => (
              <li key={query._id} className="mb-4 p-4 border rounded">
                ...
                <p><strong>Tour Details:</strong> {query.tourDetails}</p>
